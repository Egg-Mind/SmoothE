...
def ortools_solve_ilp(EClasses, EEdges, root_classes, cost, acyclic, solver,
    ...
    # if e-node i is chosen, at least one e-node from each child e-class should be chosen
    for i, edges in EEdges.items():
        for edge in edges:
            solver.Add(x[i] <= sum(x[j] for j in EClasses[edge]))

    if acyclic:
        # enforce acyclicity using topological ordering
...
def get_args():
    ...
    parser.add_argument('--fix-var', type=str, default=None)
...
def main():
    ...
    egraph = EGraphData(args.input_file, drop_self_loops=False)
    ...
    file_name = f'{file_path}/logs/ilp_log/{file_name}_ilp_solver_{args.solver}_time_{args.time_limit}.json'
